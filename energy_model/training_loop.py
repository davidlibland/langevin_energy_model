...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        for i_batch, sample_batched in enumerate(dataloader):
            ...
            noise_sample = net.sample_from_prior(replay_sample.shape[0], device=device)
            ...
            net.eval()
            model_sample = net.sample_fantasy(
                model_sample, num_mc_steps=num_mc_steps
            ).detach()
        ...
        print(
            f"on epoch {epoch}, objective: {sum(objectives) / len(objectives)}, validation metrics time: {valid_metrics_time:0.3f}"
        )
        print(
            f"total training time: {epoch_training_time:0.3f}s, total metrics time: {epoch_metrics_time:0.3f}s, validation metrics time: {valid_metrics_time:0.3f}s"
        )
