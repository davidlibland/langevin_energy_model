...
if do_search == "halving":
    ...
    distributions.add_hyperparameter(
        CSH.UniformFloatHyperparameter("lr", 1e-4, 3e-1, log=True, default_value=6e-3)
    )
    distributions.add_hyperparameter(
        CSH.UniformFloatHyperparameter(
            "sampler_lr", 1e-4, 3e-1, log=True, default_value=1e-3
        )
    )
    distributions.add_hyperparameter(
        CSH.CategoricalHyperparameter(
            "sampler",
            choices=["mala", "langevin", "tempered mala", "tempered langevin"],
        )
    )
    distributions.add_hyperparameter(
        CSH.UniformFloatHyperparameter(
            "weight_decay", 1e-4, 3e-1, log=True, default_value=5e-2
        )
    )
    distributions.add_hyperparameter(
        CSH.UniformFloatHyperparameter(
            "prior_scale", 1e-1, 10, log=True, default_value=1.12
        )
    )
    distributions.add_hyperparameter(
        CSH.UniformFloatHyperparameter("replay_prob", 0, 1, default_value=0.75)
    )
    distributions.add_hyperparameter(
        CSH.UniformFloatHyperparameter("adversary_weight", 0, 1, default_value=0.6)
    )
    distributions.add_hyperparameter(
        CSH.UniformIntegerHyperparameter("num_units", 4, 32, default_value=16)
    )
    distributions.add_hyperparameter(
        CSH.UniformIntegerHyperparameter("num_layers", 2, 4, default_value=3)
    )
    distributions.add_hyperparameter(
        CSH.UniformIntegerHyperparameter("max_replay", 2, 20, default_value=10)
    )
